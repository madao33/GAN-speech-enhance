...
class dataPreprocessor(object):
    ...
    def write_tfrecord(self):
        ...
        if tf.io.gfile.exists(self.record_path):
            print('Folder already exists: {}\n'.format(self.record_path))
        else:
            tf.io.gfile.makedirs(self.record_path)

        nlist = ["noisy/" + x for x in os.listdir(self.noisy_filelist)]
        clist = ["pure/" + x for x in os.listdir(self.clean_filelist)]
        ...
        out_file = tf.compat.v1.python_io.TFRecordWriter(self.record_path+self.record_name)
